...
class LimoMissionFSMNode(Node):
    def __init__(self):
        ...
        # Internal state
        # -------------------------
        self.state = Mission.WAIT_TRAFFIC
    ...
    def next_state(self, reason=''):
        try:
            idx = MISSION_ORDER.index(self.state)
            next_idx = min(idx + 1, len(MISSION_ORDER) - 1)
            self.set_state(MISSION_ORDER[next_idx], reason)
        except ValueError:
            self.set_state(Mission.WAIT_TRAFFIC, reason)
